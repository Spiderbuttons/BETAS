...
public static class WarpNpc
{
    ...
    {
        if (!ArgUtility.TryGet(args, 1, out string npcName, out error, allowBlank: false) || !ArgUtility.TryGet(args, 2, out string locationName, out error, allowBlank: false) || !ArgUtilityExtensions.TryGetOptionalPossiblyRelativeCoordinate(args, 3, out int x, out error) || !ArgUtilityExtensions.TryGetOptionalPossiblyRelativeCoordinate(args, 4, out int y, out error) || !ArgUtility.TryGetOptionalInt(args, 5, out int facingDirection, out error, 2))
        {
