        ...
        public static void Trigger(Farm.LightningStrikeEvent strike, bool hitRod, TerrainFeature hitFeature)
        {
            ...
            switch (hitFeature)
            {
                case HoeDirt { crop: not null } dirt:
                    strikeItem.modData["BETAS/LightningStruck/StruckCrop"] = ItemRegistry.QualifyItemId(dirt.crop.indexOfHarvest.Value);
                    break;
                case FruitTree tree:
                    strikeItem.modData["BETAS/LightningStruck/StruckTree"] = ItemRegistry.QualifyItemId(tree.treeId.Value);
                    break;
            }
        ...
        public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator il)
        {
            ...
            try
            {
                ...
                matcher.MatchEndForward(
                    ...
                    new CodeMatch(OpCodes.Ldfld, AccessTools.Field(typeof(GameLocation), nameof(GameLocation.terrainFeatures))),
                    new CodeMatch(OpCodes.Ldloca_S),
                    new CodeMatch(OpCodes.Ldloca_S),
                    new CodeMatch(OpCodes.Ldnull)
                ).ThrowIfNotMatch("Could not find TerrainFeature LocalBuilder for Utility_performLightningUpdate_Transpiler");
                ...
                matcher.MatchEndForward(
                    new CodeMatch(OpCodes.Ldloc_1),
                    new CodeMatch(m => m.opcode == OpCodes.Ldfld && m.operand is not null && m.operand.ToString()!.Contains("lightningStrikeEvent")),
