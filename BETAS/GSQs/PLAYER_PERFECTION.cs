...
public static class PlayerPerfection
{
    ...
    public static bool Items(string[] query, GameStateQueryContext context)
    {
        if (!TokenizableArgUtility.TryGetTokenizable(query, 1, out var playerKey, out var error) ||
            !TokenizableArgUtility.TryGetTokenizable(query, 2, out var calculation, out error) ||
            !TokenizableArgUtility.TryGetTokenizableInt(query, 3, out var min, out error) ||
            !TokenizableArgUtility.TryGetOptionalTokenizableInt(query, 4, out var max, out error, int.MaxValue))
        {
