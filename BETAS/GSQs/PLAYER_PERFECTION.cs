...
public static class PlayerPerfection
{
    ...
    public static bool Items(string[] query, GameStateQueryContext context)
    {
        if (!ArgUtilityExtensions.TryGetTokenizable(query, 1, out var playerKey, out var error) ||
            !ArgUtilityExtensions.TryGetTokenizable(query, 2, out var calculation, out error) ||
            !ArgUtilityExtensions.TryGetTokenizableInt(query, 3, out var min, out error) ||
            !ArgUtilityExtensions.TryGetOptionalTokenizableInt(query, 4, out var max, out error, int.MaxValue))
        {
