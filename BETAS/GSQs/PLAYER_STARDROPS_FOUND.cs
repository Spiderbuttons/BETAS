...
public static class PlayerStardropsFound
{
    ...
    {
        if (!ArgUtility.TryGet(query, 1, out var playerKey, out var error) || !ArgUtility.TryGetFloat(query, 2, out var minDrops, out error) || !ArgUtility.TryGetOptionalFloat(query, 3, out var maxDrops, out error, float.MaxValue))
        {
